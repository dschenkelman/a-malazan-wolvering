...
public class HttpClientFactory {
	public static HttpClient createClient() {
		...
		Credentials credentials = new UsernamePasswordCredentials(UserManager
				.getInstance().getEmailAddressCredential(), UserManager
				.getInstance().getPasswordCredential());
		provider.setCredentials(new AuthScope(AuthScope.ANY_HOST,
				AuthScope.ANY_PORT), credentials);
	...
	private static DefaultHttpClient getHttpClient() {
		...
		// FIX v2.2.1+ - Set timeout to 30 seconds, seems like 5 seconds was not
		// enough for good communication
		HttpConnectionParams.setConnectionTimeout(params, 30 * 1000);
		HttpConnectionParams.setSoTimeout(params, 30 * 1000);
		HttpConnectionParams.setSocketBufferSize(params, 8192);

		// Don't handle redirects -- return them to the caller. Our code often
		// wants to re-POST after a redirect, which we must do ourselves.
		...
		try {
			...
			Scheme sslTrustAllScheme = new Scheme("https", sslSocketFactory,
					443);
